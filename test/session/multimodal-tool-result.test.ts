...
//   6. Compaction pruning clears contentParts                 (src/session/methods/anchored.ts)
//
...
// ---------------------------------------------------------------------------
// Group 3 — Compaction pruning of contentParts
//
// When the anchored compaction strategy copies retained messages into the new
// session, it prunes tool outputs to '[output pruned for compaction]'. The new
// behaviour must ALSO clear contentParts so images are not forwarded.
// ---------------------------------------------------------------------------

describe("compaction — contentParts pruned alongside output", () => {
  // -----------------------------------------------------------------------
  // Test 11: pruned ToolPartData has output replaced AND contentParts cleared
  // -----------------------------------------------------------------------
  it("compaction prunes contentParts along with output for completed tool parts", () => {
    // This test validates the shape of the ToolPartData that anchored.ts must
    // write to the new session during the copy-retained phase.
    //
    // Arrange: a ToolPartData with contentParts
    const original: any = {
      tool: "screenshot",
      callId: "prune-call-001",
      status: "completed",
      input: {},
      output: "Screenshot captured.",
      contentParts: [
        { type: "text", text: "Screenshot captured." },
        { type: "image-data", data: "hugebase64blob==", mediaType: "image/png" },
      ],
    }

    // Act: simulate what anchored.ts prunedData construction MUST produce
    // (current code only sets output; new code must also clear contentParts)
    const prunedData: any = {
      ...original,
      output: "[output pruned for compaction]",
      contentParts: undefined,   // ← NEW: cleared to free image data
    }

    // Assert: output is replaced
    expect(prunedData.output).toBe("[output pruned for compaction]")

    // Assert: contentParts is gone — images must not survive compaction
    expect(prunedData.contentParts).toBeUndefined()
  })

  // -----------------------------------------------------------------------
  // Test 12: pending / running tool parts are NOT pruned (status guard)
  // -----------------------------------------------------------------------
  it("compaction does NOT prune contentParts for pending/running tool parts", () => {
    // Pending tool calls should be copied verbatim — their output is still
    // in-flight and there is nothing to prune.
    const pending: any = {
      tool: "read",
      callId: "pending-001",
      status: "pending",
    ...
    // Status guard: only completed/error are pruned
    const isPrunable = pending.status === "completed" || pending.status === "error"
    expect(isPrunable).toBe(false)

    // The data should be copied as-is for non-terminal statuses
    const copiedData = { ...pending }
    expect(copiedData.output).toBeUndefined()
    expect(copiedData.contentParts).toBeUndefined()
    expect(copiedData.status).toBe("pending")
  })

  // -----------------------------------------------------------------------
  // Test 13: toModelMessages uses pruned shape correctly
  //          (pruned contentParts → falls back to text output)
  // -----------------------------------------------------------------------
  it("pruned tool part (no contentParts) falls back to text-type output in toModelMessages", () => {
    const user = msg("user", { id: "u-prune" })
    const asst = msg("assistant", { id: "a-prune" })

    const callId = "prune-call-002"

    // Simulate the pruned ToolPartData that anchored.ts writes — no contentParts
    const prunedData: any = {
      tool: "screenshot",
      callId,
      status: "completed",
      input: {},
      output: "[output pruned for compaction]",
      // contentParts deliberately absent after pruning
    }

    const messages = [user, asst]
    const parts: PartRow[] = [
      textPart("u-prune", "take screenshot"),
      textPart("a-prune", "done"),
      {
        id: id(),
        messageId: "a-prune",
        sessionId: "s-mm",
    ...
    const toolResult = (toolMsg as any).content[0]
    // Without contentParts → falls back to string output path
    expect(toolResult.output).toEqual({
      type: "text",
      value: "[output pruned for compaction]",
    })
  })

  // -----------------------------------------------------------------------
  // Test 14: error tool part with contentParts is also pruned correctly
  // -----------------------------------------------------------------------
  it("compaction prunes contentParts for error-status tool parts too", () => {
    const errorPartWithImages: any = {
      tool: "screenshot",
      callId: "err-prune-001",
      status: "error",
      input: {},
      error: "Process timed out",
      contentParts: [
        // Partial image that was captured before the error
        { type: "image-data", data: "partialimagedata==", mediaType: "image/png" },
      ],
    }

    // Both "completed" and "error" are terminal statuses → must be pruned
    const isPrunable =
      errorPartWithImages.status === "completed" ||
      errorPartWithImages.status === "error"
    ...
    expect(prunedData.output).toBe("[output pruned for compaction]")
    expect(prunedData.contentParts).toBeUndefined()
    // Error field preserved (for debugging context)
    expect(prunedData.error).toBe("Process timed out")
  })
})
