...
/**
 * Map a configured OpenAI-compatible provider onto its credential source.
 * Lives here (not in app config) because model resolution needs it; the app
 * config supplies only the structurally-compatible `{ base_url, api_key? }`.
 */
export function providerCredentialSource(provider: { api_key?: string }): CredentialSourceConfig {
