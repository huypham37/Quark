...
/**
 * A partial map of hook name → handler.
 *
 * This is both what a {@link PluginFn} returns and the explicit, instance-scoped
 * hook map a portable runner accepts. Handlers with the same hook name in two
 * different maps never see each other.
 */
export type HookHandlers = Partial<{ [K in HookName]: HookFn<K> }>
 ...
 * import type { PluginFn } from '@quark/runner'
 *
 ...
 * ```
 */
export type PluginFn = (ctx: PluginContext) => Promise<HookHandlers>
