...
import { providerCredentialSource, type Credential } from "../provider/credentials"
import type { ConfiguredProvider } from "../provider/resolver"
...
export interface AuthServices {
  ...
  /** User-configured OpenAI-compatible providers, supplied by the app. */
  providers?: Record<string, ConfiguredProvider>
}

function definition(providerId: string, providers: Record<string, ConfiguredProvider> = {}): ProviderDefinition {
  const normalized = providerId.toLowerCase()
  const bundled = BUNDLED_PROVIDER_DEFINITIONS[normalized as keyof typeof BUNDLED_PROVIDER_DEFINITIONS]
  if (bundled) return bundled
  const custom = providers[normalized]
...
export async function loginApiKey(input: {
  ...
  const provider = definition(input.providerId, input.services?.providers)
...
export async function authStatus(services: AuthServices = {}): Promise<ProviderAuthStatus[]> {
  ...
  return Promise.all(providerIds.map((providerId) => {
    const provider = definition(providerId, providers)
    const custom = providers[providerId]
...
export async function logoutProvider(providerId: string, services: AuthServices = {}): Promise<void> {
  const provider = definition(providerId, services.providers)
