...
import { authStatus, loginApiKey, loginOAuth, logoutProvider } from "@quark/runner/commands/auth"
import { BUNDLED_PROVIDER_DEFINITIONS } from "@quark/runner/provider/definitions"
...
export async function runAuthCommand(args: string[]): Promise<number> {
  ...
  if (command === "status") {
    const statuses = await authStatus({ providers })
  ...
  if (command === "logout") {
    ...
    await logoutProvider(providerId, { providers })
  ...
  if (command === "login") {
    ...
    if (!provider) {
      const configured = providers[normalized]
      if (configured) {
        throw new Error(
    ...
    if (provider.auth.type === "oauth-device") {
      await runOAuthLogin(provider.id, provider.auth.implementation, persistence, args, providers)
    } else {
      const apiKey = await readMaskedLine(`API key for ${providerId}: `)
      await loginApiKey({ providerId, apiKey, persistence, services: { providers } })
    }
