...
import { respondQuestion } from "@quark/runner/tool/question"
import { getFiles, fuzzyFilter, clearFileCache } from "@quark/runner/shared/filelist"
...
import { collectStatistics, renderStatisticsChart } from "@quark/runner/commands/statistics"
import { StatisticsPanel } from "./statistics-panel"
import { info as notifyInfo, warn as notifyWarn } from "@quark/runner/notification/notification"
...
import { authStatus, loginApiKey, loginOAuth } from "@quark/runner/commands/auth"
...
export const App: Component<AppProps> = (props) => {
  ...
  props.bus.on("catalog-refreshed", onCatalogRefreshed)
  onCleanup(() => props.bus.off("catalog-refreshed", onCatalogRefreshed))
  ...
  const executeCommand = (commandId: string, args: string) => {
    ...
    if (props.onCommand) {
      Promise.resolve(props.onCommand(commandId, args, state.store.sessionId))
        ...
        .catch((error) => {
          notifyWarn("Command", error instanceof Error ? error.message : String(error), 4000)
        })
    }
  }
  ...
  createEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (!state.store.running && !state.store.question) {
  ...
  useKeyboard((evt) => {
    ...
    if (evt.name === "tab" && !evt.shift && !dropdownActive() && !state.store.running && pendingImages().length === 0) {
      if (props.getModels && props.getCurrentModel) {
        ...
        if (next) {
          ...
          props.bus.emit("model-switched", { modelSpec: next, thinkingEffort: "none" })
        }
      }
      evt.preventDefault()
      return
    ...
    if (evt.name === "tab" && evt.shift && !dropdownActive() && !state.store.running && pendingImages().length === 0) {
      if (props.getModels && props.getCurrentModel) {
        ...
        if (prev) {
          ...
          props.bus.emit("model-switched", { modelSpec: prev, thinkingEffort: "none" })
        }
      }
      evt.preventDefault()
      return
    ...
    // Ignored mid-run: retargeting replaces the runner, which cannot happen
    // while a run owns the abort controller.
