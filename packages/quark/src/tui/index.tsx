...
import { listWorktrees, filterToProjectWorktrees, getBranchFromPath, getWorktreeBranch, resolveWorktree, createWorktree } from "@quark/runner/worktree/worktree"
...
// --model is a runtime override (same as /model), never persisted to config.
...
// Listen for lazy session creation from the runner's prompt()
runtime.bus.on("session-created", ({ sessionId }) => {
...
const modelName = modelArg ?? activeAgent.model
...
async function handleCommand(command: string, args: string, sessionId: string | null): Promise<CommandResult> {
  ...
  if (command === "skills") {
    ...
    if (activeAgent.skills?.some((skill) => skill.name === skillName) || activatedSkillNames.has(skillName)) {
...
function reloadConfig(): Promise<void> {
  if (runtime.isBusy()) {
    bus.emit("error", {
      ...
      error: new Error("Cancel the running agent before reloading config"),
    })
...
async function openEditor(sid: string | null, target: FileTarget = { filePath: configPath() }): Promise<void> {
  ...
  try {
    ...
    if (path.resolve(target.filePath) === path.resolve(configPath())) await reloadConfig()
  } catch (err) {
    setImmediate(() => {
      bus.emit("error", {
        sessionId: sid ?? "unknown",
...
function handleGetPaletteEntries() {
  ...
  const activeSkills = new Set((activeAgent.skills ?? []).map((skill) => skill.name))
  return buildPaletteEntries({
    ...
    tools: activeAgent.tools.map((tool) => ({
      id: tool.id,
      name: tool.id,
...
render(() => (
  <App
    ...
    initialThinkingEffort={modelOverride ? "none" : activeAgent.thinkingEffort}
  />
), renderer)
